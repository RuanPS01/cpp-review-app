...
on:
  push:
    tags:
      - 'v*'
```
Dispara **somente** quando uma tag iniciada por `v` é enviada (ex: `v2.3.1`). É o mecanismo oficial de publicação de versões.
...
sequenceDiagram
    ...
    Dev->>Git: git tag v2.3.1
    Dev->>Git: git push origin v2.3.1
...
1. Atualize o campo `version` nos `package.json` (raiz, `client` e `server` devem ficar sincronizados — atualmente `2.3.1`).
