  ...
  const actionButtons = (
    <div className="flex gap-4">
        {isAnalyzing ? (
          ...
          <button
            onClick={onBack}
            className="px-6 py-2 rounded-lg border border-border-main text-text-dim font-bold uppercase text-[10px] tracking-widest hover:bg-button transition-all disabled:opacity-50"
          >
            {t.cancel}
          </button>
        )}
    ...
    <div className="bg-panel-dark border border-border-main rounded-2xl w-full min-h-[calc(100vh-140px)] flex flex-col shadow-2xl overflow-hidden shadow-accent/10 animate-in fade-in slide-in-from-left-4 duration-300">
        ...
        <div className="p-6 border-b border-border-main flex justify-between items-center bg-button/30">
          <div className="flex items-center gap-4">
            <button 
              onClick={onBack}
              className="p-2 hover:bg-accent/10 rounded-lg transition-colors text-accent flex items-center gap-2 font-bold text-xs uppercase tracking-widest"
              title={t.back}
            >
              <ArrowLeft size={20} />
              {t.back}
            ...
            <div className="flex items-center gap-3">
                ...
                <div>
                <h2 className="text-xl font-black tracking-tight text-text-bright uppercase">{t.globalAiModalTitle}</h2>
          ...
          {showConfirm ? (
            <div className="max-w-2xl mx-auto space-y-8 py-10">
              ...
              <div className="flex justify-center gap-4 w-full max-w-2xl">
                <button
                  onClick={onBack}
                  className="flex-1 py-4 rounded-xl border border-border-main text-text-dim font-black uppercase tracking-widest hover:bg-button transition-all"
                >
                  {t.cancel}
                </button>
                <button
            ...
            <div className="space-y-6">
              ...
              <div className="bg-button/10 p-4 rounded-xl border border-border-main">
              ...
              <div className="border border-border-main rounded-xl overflow-hidden bg-button/10">
                <table className="w-full text-left border-collapse">
                  ...
                  <tbody className="divide-y divide-border-main/50 text-sm">
                    {items.map((item) => (
                      <tr key={`${item.studentId}-${item.questionNum}`} className="hover:bg-white/5 transition-colors h-[52px]">
                        <td className="px-4 font-bold text-text-main">{item.studentName}</td>
                        <td className="px-4 text-center font-mono text-accent">Q{item.questionNum}</td>
                        <td className="px-4">
                          <div className="flex items-center gap-2 h-6">
                            {item.status === 'pending' && <div className="w-2 h-2 rounded-full bg-text-dim animate-pulse mx-1"></div>}
                        ...
                        <td className="px-4 text-center font-black text-text-bright">
                          {item.result?.score ?? '-'}
                        </td>
                        <td className="px-4">
